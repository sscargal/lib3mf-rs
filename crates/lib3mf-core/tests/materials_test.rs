...
<model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/2013/01/3dmodel">
    ...
    let base_mats = model.resources.get_base_materials(ResourceId(1)).expect("Base materials not found");
    assert_eq!(base_mats.materials.len(), 2);
    assert_eq!(base_mats.materials[0].name, "Red");
    assert_eq!(base_mats.materials[0].display_color, Color::new(255, 0, 0, 255));
    assert_eq!(base_mats.materials[1].name, "Green");
    assert_eq!(base_mats.materials[1].display_color, Color::new(0, 255, 0, 255));

    // Check Color Group
    let color_group = model.resources.get_color_group(ResourceId(2)).expect("Color group not found");
