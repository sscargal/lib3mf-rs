...
fn test_stats_deadpool() {
    ...
    assert!(stats.vendor.plates.len() >= 3, "Should have at least 3 plates");
    
    let main_plate = stats.vendor.plates.iter().find(|p| p.name.as_deref() == Some("Main"));
    assert!(main_plate.is_some(), "Should have 'Main' plate");
    
    let back_plate = stats.vendor.plates.iter().find(|p| p.name.as_deref() == Some("Back"));
    assert!(back_plate.is_some(), "Should have 'Back' plate");
    
    // Verify generator
    assert!(stats.generator.as_ref().unwrap().contains("Bambu"), "Should be Bambu Studio");
}
