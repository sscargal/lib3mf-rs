...
fn test_write_package_with_attachments() -> anyhow::Result<()> {
    ...
    model
        .attachments
        .insert(texture_path.to_string(), texture_data.to_vec());
    ...
    model
        .attachments
        .insert(thumb_path.to_string(), thumb_data.to_vec());
    ...
    assert!(rels_str.contains(
        "Type=\"http://schemas.microsoft.com/3dmanufacturing/2013/01/3dmodel/relationship/texture\""
    ));
