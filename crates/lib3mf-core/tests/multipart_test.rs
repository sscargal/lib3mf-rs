...
fn test_multipart_stats_recursive() {
    ...
    let root_data = archiver
        .read_entry("3D/3dmodel.model")
        .expect("Failed to read root model");
    ...
    // Vertices: 113,331
    assert!(
        stats.geometry.triangle_count > 220000,
        "Triangle count too low: {}",
        stats.geometry.triangle_count
    );
    assert!(
        stats.geometry.vertex_count > 110000,
        "Vertex count too low: {}",
        stats.geometry.vertex_count
    );
