        ...
        match parser.next()? {
            Event::Start(e) => match e.name().as_ref() {
                ...
                b"beamlattice" => {
                    ...
                    let lattice = parse_beam_lattice_content(parser, min_length, precision, clipping_mode)?;
                    mesh.beam_lattice = Some(lattice);
                }
                _ => {} // Ignore headers/metadata inside mesh for now
            },
            Event::End(e) if e.name().as_ref() == b"mesh" => break,
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in mesh".to_string())),
            _ => {}
        }
    }
...
fn parse_vertices<R: BufRead>(parser: &mut XmlParser<R>, mesh: &mut Mesh) -> Result<()> {
    loop {
        match parser.next()? {
            ...
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in vertices".to_string())),
            _ => {}
        }
    }
    Ok(())
    ...
    loop {
        match parser.next()? {
            Event::Start(e) | Event::Empty(e) if e.name().as_ref() == b"triangle" => {
                ...
                mesh.triangles.push(Triangle { v1, v2, v3, p1, p2, p3, pid });
            }
            Event::End(e) if e.name().as_ref() == b"triangles" => break,
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in triangles".to_string())),
            _ => {}
        }
    }
    Ok(())
