...
pub fn parse_beam_lattice_content<R: BufRead>(
    ...
    loop {
        match parser.next()? {
            ...
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in beamlattice".to_string())),
            _ => {}
        }
    }
    ...
    loop {
        match parser.next()? {
             Event::Start(e) | Event::Empty(e) if e.name().as_ref() == b"beam" => {
                ...
                beams.push(Beam { v1, v2, r1, r2, p1, p2, cap_mode });
            }
            Event::End(e) if e.name().as_ref() == b"beams" => break,
             Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in beams".to_string())),
            _ => {}
        }
    }
    Ok(beams)
    ...
    loop {
        ...
        match event {
            Event::Start(e) if e.name().as_ref() == b"beamset" => {
                ...
                sets.push(BeamSet { name, identifier, refs });
            }
            Event::Empty(e) if e.name().as_ref() == b"beamset" => {
                let name = get_attribute(&e, b"name");
                let identifier = get_attribute(&e, b"identifier");
                sets.push(BeamSet { name, identifier, refs: Vec::new() });
            }
            Event::End(e) if e.name().as_ref() == b"beamsets" => break,
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in beamsets".to_string())),
            _ => {}
        }
    }
    Ok(sets)
...
fn parse_refs<R: BufRead>(parser: &mut XmlParser<R>) -> Result<Vec<u32>> {
    ...
    loop {
         match parser.next()? {
            ...
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in beamset".to_string())),
            _ => {}
        }
    }
    Ok(refs)
