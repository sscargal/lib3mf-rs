    ...
    loop {
        match parser.next()? {
            Event::Start(e) => match e.local_name().as_ref() {
                ...
                b"sliceref" => { // Usually empty element <sliceref ... />?
                    // Start or Empty.
                    let stack_id = crate::model::ResourceId(get_attribute_u32(&e, b"slicestackid")?);
                    let path = get_attribute(&e, b"slicepath").unwrap_or_default();
                    refs.push(SliceRef { slice_stack_id: stack_id, slice_path: path });
                }
                _ => {}
            },
            ...
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in slicestack".to_string())),
            _ => {}
        }
    }
        ...
        match parser.next()? {
            Event::Start(e) => match e.local_name().as_ref() {
                ...
                b"polygon" => {
                    ...
                    polygons.push(Polygon { start_segment: start, segments });
                }
                _ => {}
            },
            Event::End(e) if e.local_name().as_ref() == b"slice" => break,
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in slice".to_string())),
            _ => {}
        }
    }

    Ok(Slice { z_top, vertices, polygons })
}
...
fn parse_slice_vertices<R: BufRead>(parser: &mut XmlParser<R>) -> Result<Vec<Vertex2D>> {
    ...
    loop {
        match parser.next()? {
            ...
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in slice vertices".to_string())),
            _ => {}
        }
    }
    Ok(vertices)
...
fn parse_polygon_segments<R: BufRead>(parser: &mut XmlParser<R>) -> Result<Vec<Segment>> {
    ...
    loop {
        match parser.next()? {
            ...
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in polygon segments".to_string())),
            _ => {}
        }
    }
    Ok(segments)
