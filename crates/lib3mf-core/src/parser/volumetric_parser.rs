...
use crate::model::{VolumetricStack, VolumetricLayer, VolumetricRef, ResourceId};
        ...
        match parser.next()? {
            Event::Start(e) => {
                 ...
                 match local_name.as_ref() {
                    ...
                    b"volumetricref" => {
                        let stack_id = get_attribute_u32(&e, b"volumetricstackid").map(ResourceId)?;
                        let path = get_attribute(&e, b"path").unwrap_or_default();
                        let end_tag = e.name().as_ref().to_vec();
            ...
            Event::Eof => return Err(Lib3mfError::Validation("Unexpected EOF in volumetricstack".to_string())),
            _ => {}
        }
    }
