        ...
        while reader.read_line(&mut line).map_err(Lib3mfError::Io)? > 0 {
            ...
            match parts[0] {
                "v" => {
                    ...
                    let x = parts[1].parse::<f32>().map_err(|_| Lib3mfError::Validation("Invalid float".to_string()))?;
                    let y = parts[2].parse::<f32>().map_err(|_| Lib3mfError::Validation("Invalid float".to_string()))?;
                    let z = parts[3].parse::<f32>().map_err(|_| Lib3mfError::Validation("Invalid float".to_string()))?;
                ...
                "f" => {
                    ...
                    for part in &parts[1..] {
                        ...
                        let v_idx = v_idx_str.parse::<i32>().map_err(|_| Lib3mfError::Validation("Invalid index".to_string()))?;
                        
                        let idx = if v_idx > 0 {
                            (v_idx - 1) as u32
                        } else {
                            // Relative index
                             return Err(Lib3mfError::Validation("Relative OBJ indices not supported yet".to_string()));
                        };
                        indices.push(idx);
                    }
        ...
        for item in &model.build.items {
             if let Some(object) = model.resources.get_object(item.object_id) {
                 if let lib3mf_core::model::Geometry::Mesh(mesh) = &object.geometry {
                      ...
                      writeln!(writer, "g {}", object.name.as_deref().unwrap_or("Object")).map_err(Lib3mfError::Io)?;
