...
pub async fn load_model_async<P: AsRef<Path>>(path: P) -> Result<Model> {
    ...
    let model_rel = rels.iter().find(|r| r.target_mode == "Internal" && 
         r.rel_type == "http://schemas.microsoft.com/3dmanufacturing/2013/01/3dmodel")
         .ok_or(Lib3mfError::InvalidStructure("No 3D Model part found in .rels".to_string()))?;
    ...
    let model = tokio::task::spawn_blocking(move || {
        ...
        parse_model(cursor)
    }).await.map_err(|e| Lib3mfError::Validation(format!("Join error: {}", e)))??; // JoinError + Parse Result
